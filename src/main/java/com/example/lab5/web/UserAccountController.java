...
import com.example.lab5.service.UserAccountService;
...
import com.example.lab5.web.mapper.PostMapper;
import com.example.lab5.web.mapper.TopicMapper;
import com.example.lab5.web.mapper.UserAccountMapper;
...
public class UserAccountController {
      ...
      @ApiResponse(responseCode = "200",
          content = {
              @Content(schema = @Schema(
                  implementation = UserAccountViewDto.class))
          })
  })
  public ResponseEntity<List<UserAccountViewDto>> findAll() {
  ...
  @Operation(summary = "Get user by id", responses = {
      @ApiResponse(responseCode = "200",
          content = {
              @Content(schema = @Schema(implementation = UserAccountViewDto.class))
          }),
      @ApiResponse(responseCode = "404", description = "User not found",
          content = @Content)
  })
  public ResponseEntity<UserAccountViewDto> findCreationForm(@PathVariable Long id) {
  ...
  public ResponseEntity<List<PostViewDto>> findAllPostsByUser(@PathVariable Long id) {
    return ResponseEntity.of(userAccountService.findById(id).map(
        userAccount -> userAccountService.findAllByPosts(userAccount).stream()
  ...
  public ResponseEntity<List<TopicViewDto>> findAllTopicsByUser(@PathVariable Long id) {
    return ResponseEntity.of(userAccountService.findById(id)
            .map(userAccount -> userAccountService.findAllTopics(userAccount).stream()
  ...
  public ResponseEntity<UserAccountViewDto> findUpdateForm(@PathVariable Long id,
                                                    @RequestBody UserAccountModifyDto modifyDto) {
    return ResponseEntity.of(userAccountService.findById(id)
  ...
  public ResponseEntity<UserAccountViewDto> create(@RequestBody UserAccountModifyDto userAccount) {
